...
- `Provider.supports_tool_result_media`: whether an `Image`/`File` block inside a `ToolResponse` reaches the model in place. True for Anthropic-family, Bedrock, and Gemini (native tool-result media); False for the OpenAI-compat family, which instead promotes an `Image` into a synthetic user turn appended after the tool row (`providers/_openai_compat.py::messages_to_openai`) — `File` (PDF) results are never promoted, since Chat Completions has no document wire type.
