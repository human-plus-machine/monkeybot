...
2. **Capture verdicts in the eval runner.** `evals/runner.py` collects SSE events into `TurnResult` (`evals/models.py`) and already counts `ContextSummarized` as `summarizations_count`. Add `verdicts: list[VerdictRecord]` (status, severity, triggering signals) populated from the `VerifierVerdict` event once Phase 2 emits it. Add a structured `tool_calls[].path_args: list[str]` field extracted from path-like keys (`path`, `file_path`, `paths`) — `args_summary` is truncated to 300 chars and is not glob-matchable.
3. **New assertion keys** in `evals/assertions.py` (live) and `tests/evals/scenario_runner.py::ScenarioAssertions` (deterministic; evaluated in `tests/evals/test_agent_behavior.py`): `min_verdicts`, `max_verdicts`, `verdict_status_in`, `verdict_severity_max`, and `files_not_touched` (a glob list checked against every recorded tool call's `path_args`). The deterministic runner additionally needs `system_prompt_contains_once` and `tools_empty_on_turn` so Part 9's "nudge lands exactly once" and "replan empties tools for one turn" become YAML assertions rather than hand-written tests.
