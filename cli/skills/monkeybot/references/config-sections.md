...
Deep reference for every `monkeybot.yaml` section. Load this only when a user needs to customize beyond Tier 1. The canonical, fully-commented template lives at `cli/src/monkeybot_cli/templates/monkeybot.example.yaml`; this file adds the **"when would I change this?"** context the comments don't.
