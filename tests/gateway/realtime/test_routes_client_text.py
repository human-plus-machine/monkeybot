...
@pytest.mark.asyncio
async def test_assistant_boundary_pins_tool_executor_to_connection_config(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The tool executor built per boundary must use ``ctx.config``, not a fresh read.

    Otherwise a config reload mid-connection lets later boundaries pick up new
    sandbox/tool settings while the connection's provider session and context
    stay pinned to the revision it opened with.
    """
    state = _state()
    state.buffer.add_user_text("hello")
    state.buffer.mark_user_turn_boundary()
    ...
    history.append = AsyncMock()
    history.load = AsyncMock(return_value=[])
    pinned_config = object()
    ctx = MagicMock()
    ctx.memory = None
    ctx.thread_id = "s1"
    ctx.request_id = "r1"
    ctx.model = "fake"
    ctx.config = pinned_config
    deps = MagicMock()
    deps.inspectors = []
    deps.hook_manager = None
    ...
    monkeypatch.setattr("monkeybot.gateway.realtime.routes._create_tool_executor", _spy)

    await _handle_assistant_boundary(MagicMock(), state, ctx, history, deps, None, None)

    assert seen_configs == [pinned_config]
