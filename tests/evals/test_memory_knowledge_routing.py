...
def test_harness_memory_paths_and_no_delegation() -> None:
    text = harness_fixed_context(
        include_task_tool=True,
        workspace_root="/tmp/ws",
        memory_storage_uri="local://./memory/mempalace",
    )
    assert "### Memory retrieval (`mempalace search`)" not in text
    assert "search_memory" not in text
    assert (
        "do not `read_file` palace paths" in text.lower()
        or "do not read_file palace" in text.lower()
    )
    assert "outside" in text.lower()
    assert "mempalace search" in text.lower()
