...
async def _run_worker_capture_thread_id(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    *,
    envelope: SubagentEnvelope,
) -> str:
    """Run worker with fakes; return the ``thread_id`` passed to ``build_context``."""
    from monkeybot.core.subagents import subagent_worker
...
@pytest.mark.asyncio
async def test_worker_uses_envelope_child_thread_id(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    fixed = "subagent:parent-sess:fixedid0001"
    envelope = SubagentEnvelope(
        task="say ok",
        context="",
        memory_storage_uri="",
        ...
        model="gemini-2.5-flash",
        child_thread_id=fixed,
    )
    got = await _run_worker_capture_thread_id(tmp_path, monkeypatch, envelope=envelope)
    assert got == fixed


@pytest.mark.asyncio
async def test_worker_generates_thread_id_when_missing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import re
    ...
    assert envelope.child_thread_id is None
    got = await _run_worker_capture_thread_id(tmp_path, monkeypatch, envelope=envelope)
    assert re.fullmatch(r"subagent:parent-sess:[0-9a-f]{10}", got)
