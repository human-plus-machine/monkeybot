...
@pytest.mark.asyncio
async def test_create_harness_deps_skips_memory_when_disabled(
...
def test_context_window_tokens_reads_model_env(monkeypatch: pytest.MonkeyPatch) -> None:
    from monkeybot.core.config.snapshot import context_window_tokens

    monkeypatch.setenv("MODEL_CONTEXT_WINDOW", "750000")
    assert context_window_tokens() == 750_000
