...
FIRESTORE_PROJECT="${FIRESTORE_PROJECT:-monkeybot-demo}"
...
OBS_COLLECTOR_CONFIG_PHOENIX="${OBS_COLLECTOR_CONFIG_PHOENIX:-$(pwd)/monkeybot_config/otel-collector.demo.yaml}"
OBS_COLLECTOR_CONFIG_DUAL="${OBS_COLLECTOR_CONFIG_DUAL:-$(pwd)/monkeybot_config/otel-collector.demo-dual.yaml}"
...
ensure_demo_sandbox_worker_image() {
...
# never leave the demo agent stuck on a stale DB (skipped when using Firestore).
if _demo_uses_sqlite; then
  rm -f \
...
if [[ -f .env ]]; then
  ...
  uv run -m monkeybot.gateway.realtime_main || exit_code=$?
fi
exit "${exit_code}"
