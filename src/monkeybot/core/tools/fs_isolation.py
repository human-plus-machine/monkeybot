...
def _probe(mechanism: str) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="monkeybot-isolation-probe-") as tmp:
        ...
        inner = [sys.executable, "-c", check, str(secret_file)]
        hidden = [str(secret_dir)]
        argv = (
            _namespace_argv(inner[0], inner[1:], hidden)
            if mechanism == "namespace"
            else _sandbox_exec_argv(inner[0], inner[1:], hidden)
        )
        try:
            proc = subprocess.run(
                argv,
...
def isolated_argv(
    ...
    *,
    ...
    Missing paths are still passed through: the child must not be able to
    observe the directory even if it is created between planning and exec.
    """
    if not support.available:
        raise ValueError(f"isolation is unavailable: {support.detail}")
    hidden = [str(Path(path)) for path in hidden_paths]
    if not hidden:
