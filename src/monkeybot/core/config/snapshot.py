...
class ConfigStore:
    ...
    def reload(
        ...
        published = replace(built, revision=revision)
        diff = (
            diff_runtime_configs(current, published)
            if current is not None
            else ConfigDiff(
                noop=False,
                changed_env_keys=frozenset(published.env_values),
                changed_content=frozenset(),
                tiers=frozenset(ENV_TIERS[k] for k in published.env_values if k in ENV_TIERS),
            )
        )
