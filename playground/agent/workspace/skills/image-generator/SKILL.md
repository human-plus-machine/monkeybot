...
- If the script reports a missing dependency (e.g. `google-genai package not installed`), **do not** run `pip install`, `uv pip install`, or any other package install via `run_command` — installs are blocked by policy. In the playground, `google-genai` is preinstalled in the gateway venv and sandbox worker image; ask the user to restart `./run.sh` if the error persists.
