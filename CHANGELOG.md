...
- Browser MCP Phase 5: `browser_act` runs up to 25 sequential steps in one turn; `browser_fill_form` fills by label; `browser_click_text` clicks by visible text; `browser_extract` scrapes structured rows. Shared `do_*` helpers back both the single tools and the batch executor.
- Browser MCP Phase 6: optional `playbook` YAML fences in host markdown are executable via `browser_run_playbook`. `browser_list_playbooks` / `browser_goto` list `flows`. `browser_write_playbook` validates fences before save. `browser_recent_actions` returns the last 50 successful actions per host (labels and lengths, never typed text). Timeout: `BROWSER_MCP_PLAYBOOK_TIMEOUT_S` (default 120). Secrets are never flow params — use `{do: login, expected_origin}`.
