...
### Changed
- **Documentation (v2):** Removed legacy published guides (OpenClaw `docs/ref/`,
  harness extension suite under `docs/harness/`, scheduler, v1 agent/deploy/integrations
  pages). Current developer docs are `docs/getting-started.md`, `docs/skills.md`,
...
- **Public contract-test suite.** `monkeybot.core.harness.extensions.testing`
...
- **Canonical DynamoDB Checkpointer example** at
  ...
  `monkeybot.checkpointers` entry point and drives the framework contract suite
  under `moto`.
...
### Removed
- `monkeybot.core.harness.checkpointer.DynamoDBCheckpointerStub` — the legacy
...
- **OpenAI / Anthropic API keys are wrapped in `pydantic.SecretStr`** at
  ...
  structured logging.
- **Legacy `monkeybot.core.harness.checkpointer.FirestoreCheckpointer` is
